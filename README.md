...
- Complex type conversions (e.g., YAML → Python dataclasses)
...
```
my_project/
├── prism.project.yaml    # Project configuration
├── configs/              # Base configurations
│   ├── base.yaml
│   ├── validator.py      # Optional: custom validator
│   └── prism/            # Sweep definitions
│       ├── lr_sweep.prism.yaml
│       └── model_sweep.prism.yaml
├── scripts/
│   └── train.py          # Your training script
└── outputs/              # Results and state
    ├── my_study.prism    # Study state file
    └── my_study/         # Experiment outputs
```
...
- 📊 Study overview with experiment status
